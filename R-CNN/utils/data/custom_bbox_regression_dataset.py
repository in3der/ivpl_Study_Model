...
"""
 Bounding box regressor 학습 데이터 생성
"""
...
class BBoxRegressionDataset(Dataset):
    ...
    def __init__(self, root_dir, transform=None):
        ...
        jpeg_list = list()
        # 저장하는 format {'image_id': ?, 'positive': ?, 'bndbox': ?}
        box_list = list()
    ...
    def __getitem__(self, index: int):
        assert index < self.__len__(), '데이터 세트 크기: %d, 현재 입력 첨자: %d' % (self.__len__(), index)
    ...
    def get_bndbox(self, bndboxes, positive):
        """
        가장 큰 IoU가 있는 annotated bounding box를 반환
        :param bndboxes: 크기 [N, 4] 또는 [4]
        :param positive: 크기 [4]
        :return: [4]
        """

        if len(bndboxes.shape) == 1:
            # annotated bounding box가 1개 있는 경우 직접 반환함
            return bndboxes
        else:
...
def test2():
    ...
    data_root_dir = '/home/ivpl-d29/dataset/VOC/voc_car/bbox_regression'
    data_set = BBoxRegressionDataset(data_root_dir, transform=transform)
